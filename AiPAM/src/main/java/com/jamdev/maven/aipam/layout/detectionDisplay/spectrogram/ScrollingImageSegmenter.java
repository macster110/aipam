...
public abstract class ScrollingImageSegmenter {
	 ...
	 */
	public abstract void drawADataUnit(GraphicsContext g, PAMDataUnit  pamDataUnit, WritableImageSegment writableImage, PlotProjector tdProjector, double scrollStart);
	 ...
	 * @param scrollStart - the start of the display in milliseconds 
	 */
	public abstract Path2D getPath2D(PAMDataUnit  pamDataUnit, WritableImageSegment writableImage, PlotProjector tdProjector, double scrollStart);
	 ...
	 */
	public Path2D drawDataUnit(GraphicsContext g, PAMDataUnit  pamDataUnit, PlotProjector tdProjector, double scrollStart) {
		...
		boolean checkImage = checkMillisPerPixel(tdProjector.getXPixSize(), tdProjector.getXAxis().getMaxVal()); 
		...
		if (bufferCheck) {
			this.bufferCheck(scrollStart, tdProjector.getXAxis().getMaxVal()); 
			bufferCheck=false; 
		}
...
//								+ " writableImageChunk: " + imageSegments.get(i).hashCode() + " N: " + imageSegments.size()); 
			...
			tC = tdProjector.getXPix(tcMillis);
			tCEnd = tdProjector.getXPix(tcMillis + imageSegments.get(i).getWidth()*millisPerPixel);
