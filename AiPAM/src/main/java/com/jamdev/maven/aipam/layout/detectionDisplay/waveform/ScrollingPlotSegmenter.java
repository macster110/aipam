...
public abstract class ScrollingPlotSegmenter {
	 ...
	 * @param dataUnitStartTime - the start time in millis 
	 * @param dataUnitEndTime - the end time in millis. 
...
//				System.out.println("3 Make a new image: " + imagePixelTime + " N segments: " + imageSegments.size() + " imagePixelTime: " + imagePixelTime + " pixelstart " +  (int) Math.floor(imagePixelTime/imageWidth)*imageWidth); 
	 ...
	 */
	public abstract void addPlotData(PAMDataUnit  pamDataUnit, PlotSegment plotSegmentData, PlotProjector tdProjector, double scrollStart);
	 ...
	 * @param scrollStart - the start of the display in milliseconds. 
	 */
	public abstract void paintPlotData(GraphicsContext g, PlotSegment plotSegmentData, PlotProjector tdProjector, double scrollStart);
	 ...
	 */
	public void paintImages(GraphicsContext g, PlotProjector tdProjector, double scrollStart, long tm) {
		
		if (tm==0) {
			paintImages(g, tdProjector, scrollStart);
			return;
		}
		//System.out.println("Paint images start");
...
//				" displayWidthPixels: " + displayWidthPixels);
		...
		boolean checkImage = checkMillisPerPixel(tdProjector.getXPixSize(), (tdProjector.getXAxis().getMaxVal()- tdProjector.getXAxis().getMinVal())/1000.); 
		...
		//System.out.println("Plot segments:  " + plotSegments.size() + "  " + pamDataUnit.getDurationInMilliseconds());
		...
		for (PlotSegment aPlotSegment :  plotSegments) {
			...
			if (bufferCheck) {
				this.bufferCheck(scrollStart, (tdProjector.getXAxis().getMaxVal()-tdProjector.getXAxis().getMinVal())/1000.); 
				bufferCheck=false; 
			}
		...
		//paintImages(g, tdProjector, scrollStart, 0); 
		return null; //TODO - should always be returning a path2D. 
	}
