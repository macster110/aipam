...
public class HidingPane extends StackPane {
	 ...
	 */
	private int iconSize=AIPamView.iconSize;
...
//		mainPane.setCenter(hidePane);
	...
	private Button createShowButton(final boolean show){
		final Button pamButton=new Button();
...
//			  if (show && Math.abs(distance)>expandedSize) return; 
	 ...
	 */
	private void setHideButtonPos(Side side){
		switch (this.side){
		case RIGHT:
